...
infeasibility_analysis <- function(n_start, n_end, n_interval, s_start, s_end){
	...
	for(n in seq(n_start, n_end, n_interval)){
		...
		for(seed in s_start:s_end){
			...
			con <- file(paste0("data/STP_output/random_n",n,"_s",seed,"_output_STP_format_1-2.txt"), "r")
		...
		infeasible_1_2 <- 100 - infeasible_1_2 / (s_end - s_start + 1) * 100
		infeasible_1_2_3 <- 100 - infeasible_1_2_3 / (s_end - s_start + 1)  * 100	
		percent_1_2 <- c(percent_1_2, infeasible_1_2)
		percent_1_2_3 <- c(percent_1_2_3, infeasible_1_2_3)	
	}
	
	x <- seq(n_start,n_end,n_interval)
	png(paste0("data/analysis_plots/n",n_start,"<-",n_interval,"->",n_end,"_s",s_start,"<->",s_end,"_infeasibility_percentage.png"), width = 512, height = 384)
